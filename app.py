...
# Etapas validas del embudo comercial (Sandra automatiza 1-4)
ETAPAS_VALIDAS = [
    "nuevo_contacto", "pidio_info", "acepto_propuesta_cita", "cita_agendada",
    "no_califica", "perdido"
]
...
# ============================================================
EVOLUTION_URL = os.getenv("EVOLUTION_URL", "http://izoltek_evolution-api:8080/message/sendMedia/Salvadornavarrobienesraices")
EVOLUTION_APIKEY = os.getenv("EVOLUTION_APIKEY", "429683C4C977415CAAFCCE10F7D57E11")
...
def agregar_a_blacklist(telefono, nombre=None, motivo="agregado por Salvador"):
    """Agrega un numero a la blacklist. Devuelve True si se agrego."""
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO sandra_blacklist (telefono, nombre, motivo)
            VALUES (%s, %s, %s)
        ...
        print(f"[BD] Error agregar blacklist: {e}")
        return False


def quitar_de_blacklist(telefono):
    """Quita un numero de la blacklist. Devuelve True si se quito algo."""
    tel = normalizar_telefono(telefono)
    if not tel:
        return False
    conn = get_db_connection()
    ...
    try:
        cur = conn.cursor()
        cur.execute("DELETE FROM sandra_blacklist WHERE telefono = %s", (tel,))
        ...
        print(f"[BD] Error quitar blacklist: {e}")
        return False
...
def registrar_kit_enviado(usuario_id):
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO conversaciones (usuario_id, sender_id, rol, mensaje, canal, timestamp)
            VALUES (%s, %s, 'sandra', %s, 'sistema', NOW())
        """, (usuario_id, usuario_id, "[KIT_FOTOS_ENVIADO]"))
        conn.commit()
        cur.close()
        conn.close()
        return True
...
TOOLS_AGENTE = [
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "usuario_id": {"type": "string"},
                "motivo": {"type": "string", "description": "lead_caliente | cita_agendada | tema_complejo | pidio_humano | tema_personal"},
    ...
    {
        "name": "enviar_fotos_castana",
        "description": "Envia las 16 fotos del departamento Castaña al cliente por WhatsApp. SOLO usar cuando el cliente ACEPTE verlas (le ofreciste y dijo 'si', 'mandalas', 'va', 'claro', etc). NUNCA mandar sin que el cliente confirme. Es un solo envio: NO repetir aunque el cliente vuelva a pedir.",
        "input_schema": {
            "type": "object",
            "properties": {
                "usuario_id": {"type": "string"}
...
TOOLS_ASISTENTE = [
    ...
    {
        "name": "agregar_blacklist",
        "description": "Agrega un numero a la blacklist para que Sandra NUNCA le responda. Usar cuando Salvador diga 'agrega a blacklist el numero X', 'bloquea a X', etc.",
        "input_schema": {
            "type": "object",
            "properties": {
                "telefono": {"type": "string", "description": "Numero a bloquear (cualquier formato)"},
                "nombre": {"type": "string", "description": "Nombre del contacto, si Salvador lo menciona"}
            },
            "required": ["telefono"]
        }
    },
    {
        "name": "quitar_blacklist",
        "description": "Quita un numero de la blacklist para que Sandra vuelva a responderle. Usar cuando Salvador diga 'quita de blacklist a X', 'desbloquea a X', etc.",
        "input_schema": {
            "type": "object",
            "properties": {
...
def tool_agendar_visita(usuario_id, fecha, hora, desarrollo="Castaña", nombre_cliente=None, es_caliente=False, necesita_traslado=False, conversation_id=None):
    # GATE 2: no agendar sin fecha y hora reales (evita falsos positivos)
    placeholders = ("", "none", "null", "pendiente", "por confirmar", "tbd", "n/a")
    if not fecha or str(fecha).strip().lower() in placeholders:
        return {"error": "falta_fecha", "instruccion": "Pregunta al cliente que dia exacto quiere la visita antes de agendar."}
    if not hora or str(hora).strip().lower() in placeholders:
        return {"error": "falta_hora", "instruccion": "Pregunta al cliente a que hora quiere la visita antes de agendar."}
    ...
    try:
        ...
        # Construir fecha_cita como timestamp real (fecha + hora).
        # Si fecha no trae formato fecha completo, se guarda lo que venga y se complementa con hora.
        fecha_cita_str = f"{str(fecha).strip()} {str(hora).strip()}"

        # Mapear extras (sin columnas propias) a notas legibles
        notas_extra = []
        notas_extra.append(f"Desarrollo: {desarrollo}")
        if es_caliente:
            notas_extra.append("LEAD CALIENTE")
        if necesita_traslado:
            notas_extra.append("Necesita traslado desde estacion Lomas del Sur")
        notas_str = " | ".join(notas_extra)

        tel = normalizar_telefono(usuario_id)
        cur.execute("""
            INSERT INTO citas (sender_id, nombre, telefono, fecha_cita, estatus, notas, atendida_por, fecha_creacion)
            VALUES (%s, %s, %s, %s, 'agendada', %s, 'Sandra', NOW())
            RETURNING id
        """, (usuario_id, nombre_cliente, tel, fecha_cita_str, notas_str))
        ...
        encolar_notificacion(ADMIN_PHONE, msg, tipo="cita_agendada")

        # GATE 3: agendar_visita es la autoridad unica del evento "cita".
        # Ya notifico arriba; aqui pausa para que Salvador tome la cita.
        # (Se elimino el escalar_a_salvador post-cita del prompt para no duplicar.)
        if conversation_id:
            pausar_sandra_para(conversation_id, usuario_id, "Cita agendada")
        else:
            pausar_sandra_para(f"user_{usuario_id}", usuario_id, "Cita agendada")
...
def tool_guardar_lead_update(usuario_id, nombre=None, telefono=None, etapa=None,
    ...
    # GATE 1: validar etapa. Si Claude manda una etapa invalida, se ignora
    # (se deja en None para no escribir basura en BD; las demas columnas se guardan).
    if etapa and etapa not in ETAPAS_VALIDAS:
        print(f"[GATE1] Etapa invalida ignorada: '{etapa}'")
        etapa = None
...
def tool_escalar_a_salvador(usuario_id, motivo, resumen, urgente=False, conversation_id=None):
    ...
    encolar_notificacion(ADMIN_PHONE, msg, tipo=motivo)
    return {
        "escalado": True,
...
def tool_enviar_fotos_castana(usuario_id):
    """Envia el kit de 16 fotos al cliente, UNA sola vez por cliente.
    Verifica en BD si ya se enviaron antes (anti-spam confiable)."""
    # Anti-spam: revisar directo en BD si ya hay marcador
    conn = get_db_connection()
    if conn:
        try:
            cur = conn.cursor()
            cur.execute("""
                ...
                WHERE usuario_id = %s AND mensaje = '[KIT_FOTOS_ENVIADO]'
                LIMIT 1
            """, (usuario_id,))
            ya_enviado = cur.fetchone() is not None
            cur.close()
            conn.close()
            if ya_enviado:
                return {
                    ...
                    "instruccion": "Las fotos YA se enviaron antes a este cliente. NO las reenvies. Dile que revise mas arriba en el chat o continua la conversacion."
                }
        except Exception as e:
            print(f"[KIT] Error verificando anti-spam: {e}")

    enviadas = enviar_kit_castana(usuario_id)
    if enviadas > 0:
        registrar_kit_enviado(usuario_id)
        return {
            "enviado": True,
            "total_fotos": enviadas,
            "instruccion": "Fotos enviadas. Avisa al cliente que ya le llegaron las fotos y pregunta si tiene dudas o quiere agendar visita."
        }
    return {
        "enviado": False,
        "instruccion": "Hubo un problema enviando las fotos. Pide disculpas y ofrece agendar visita para que las vea en persona."
    }
...
def tool_reporte_citas_agendadas(rango):
    ...
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        if rango == "hoy":
            cur.execute("SELECT * FROM citas WHERE DATE(fecha_cita) = CURRENT_DATE ORDER BY fecha_cita")
        elif rango == "manana":
            cur.execute("SELECT * FROM citas WHERE DATE(fecha_cita) = CURRENT_DATE + INTERVAL '1 day' ORDER BY fecha_cita")
        elif rango == "semana":
            cur.execute("SELECT * FROM citas WHERE DATE(fecha_cita) BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days' ORDER BY fecha_cita")
        else:
            cur.execute("SELECT * FROM citas WHERE DATE(fecha_cita) >= CURRENT_DATE ORDER BY fecha_cita LIMIT 30")
        rows = [dict(r) for r in cur.fetchall()]
        cur.close()
        conn.close()
        for r in rows:
...
def tool_resumen_dia():
    ...
    try:
        ...
        # Citas hoy
        cur.execute("SELECT COUNT(*) FROM citas WHERE DATE(fecha_cita) = CURRENT_DATE")
...
def procesar_herramienta(nombre, params, contexto_usuario=None, contexto_conversation_id=None):
    try:
        ...
        elif nombre == "enviar_fotos_castana":
            return tool_enviar_fotos_castana(
                usuario_id=params.get("usuario_id") or contexto_usuario
            )
        ...
        elif nombre == "agregar_blacklist":
            ok = agregar_a_blacklist(
                telefono=params.get("telefono"),
                nombre=params.get("nombre")
            )
            tel = normalizar_telefono(params.get("telefono"))
            if ok:
                return {"ok": True, "mensaje": f"Listo, el numero {tel} quedo en la blacklist. Sandra no le respondera mas."}
            return {"ok": False, "mensaje": f"No pude agregar el numero {tel}. Revisa que este bien escrito."}
        elif nombre == "quitar_blacklist":
            ok = quitar_de_blacklist(telefono=params.get("telefono"))
            tel = normalizar_telefono(params.get("telefono"))
            if ok:
...
- Cuando el cliente muestre interes, OFRECELE las fotos: "¿Te gustaria que te mande unas fotos del departamento? 📸"
- SOLO usa enviar_fotos_castana cuando el cliente DIGA QUE SI (responde "si", "claro", "mandalas", "va", "porfa", etc).
- NUNCA mandes las fotos sin que el cliente acepte primero.
- Es UN solo envio. Si el cliente vuelve a pedir fotos, NO las reenvies: la herramienta te avisara si ya se enviaron, dile que las revise mas arriba en el chat.

## TONO PERSONAL 👤
Tu rol es SOLO sobre temas inmobiliarios (Castaña, propiedades, creditos, visitas).
Si el cliente cambia a un tono PERSONAL o tema NO inmobiliario (te coquetea, pregunta cosas tuyas, propone verse fuera del contexto de venta, platica personal, confianza excesiva):
1. Responde algo breve y amable: "Que amable 😊 Mejor le aviso a Salvador para que el te atienda personalmente, en un momento te contacta."
2. Usa escalar_a_salvador con motivo "tema_personal" y un resumen de lo que paso.
NO sigas la platica personal. Tu trabajo es vender, no hacer conversacion personal.
...
2. Agenda para ESE momento que pide el cliente
3. Dale prioridad y trato de urgencia en tu respuesta
(El sistema marca y escala automaticamente; tu solo enfocate en cerrar rapido y con tono adecuado.)
...
(Al agendar, el sistema notifica a Salvador y pausa automaticamente. NO necesitas escalar despues de agendar.)
...
### Blacklist:
- "agrega a blacklist el numero X" / "bloquea a X" -> agregar_blacklist
- "quita de blacklist a X" / "desbloquea a X" -> quitar_blacklist
...
def chat():
    ...
    # ============================================================
    # FILTRO OUTBOUND: ¿Es un mensaje SALIENTE (de Sandra/Salvador)?
    # Defensa anti-loop de 2do nivel. Lee varios nombres de campo
    # por compatibilidad; si n8n no manda el campo, no afecta el flujo.
    # ============================================================
    es_saliente = (
        data.get("from_me") is True
    ...
    if es_saliente:
        return jsonify({
            "respuesta": "",
            "ignorar": True,
            "motivo_ignorar": "mensaje_saliente"
        })

    # ============================================================
    # FILTRO ANTI-LOOP: Sandra JAMAS se responde a si misma
    # ============================================================
    if normalizar_telefono(usuario_id) == SANDRA_PHONE:
        return jsonify({
            "respuesta": "",
            "ignorar": True,
            "motivo_ignorar": "mensaje_de_sandra"
        })

    # ============================================================
    # FILTRO GRUPOS: ¿Es chat grupal (@g.us)? -> IGNORAR TOTALMENTE
    # Sandra JAMAS responde grupos: no IA, no lead, no nada.
    # ============================================================
    if "@g.us" in str(usuario_id):
        return jsonify({
            "respuesta": "",
            "ignorar": True,
            "motivo_ignorar": "chat_grupal"
        })
    ...
    # (Responde a TODOS los clientes nuevos; la blacklist los filtra)
    # ============================================================
...
def chat_modo_agente(usuario_id, mensaje_usuario, historial_previo, conversation_id=None):
    ...
    # Si detectamos palabra caliente, pegarle una nota de TONO al mensaje
    mensaje_con_alerta = mensaje_usuario
    if es_caliente_mensaje:
        mensaje_con_alerta = f"{mensaje_usuario}\n\n[NOTA INTERNA SISTEMA: El cliente uso palabras de URGENCIA INMEDIATA. Dale prioridad y tono de urgencia: NO empujes la cita a fin de semana, cierra hoy o mañana segun pidio. El sistema ya lo marca y escala automaticamente.]"
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        ...
                        if not tool_input.get("usuario_id") and block.name in (
                            ...
                            "enviar_fotos_castana"
                        ):
                            tool_input["usuario_id"] = usuario_id

                        # ============================================================
                        # AUTORIDAD PYTHON: el flag caliente NO lo decide Claude.
                        # Python sobreescribe segun detectar_palabra_caliente().
                        # ============================================================
                        if block.name == "guardar_lead_update":
                            tool_input["caliente"] = es_caliente_mensaje
                        elif block.name == "agendar_visita":
                            tool_input["es_caliente"] = es_caliente_mensaje
                        elif block.name == "escalar_a_salvador":
                            # Si es caliente, el escalamiento es urgente si o si
                            if es_caliente_mensaje:
                                tool_input["urgente"] = True
                        ...
                        if block.name == "escalar_a_salvador":
                            sandra_pausada = True
                        if block.name == "agendar_visita" and isinstance(resultado, dict) and resultado.get("sandra_pausada"):
                            sandra_pausada = True
...
def trigger_resumen_diario():
    ...
    try:
        ...
        # Citas hoy
        cur.execute("""
            SELECT nombre AS nombre_cliente, telefono, fecha_cita, estatus, notas
            FROM citas WHERE DATE(fecha_cita) = CURRENT_DATE
            ORDER BY fecha_cita
        """)
        ...
        cur.execute("""
            SELECT nombre AS nombre_cliente, telefono, fecha_cita, estatus, notas
            FROM citas WHERE DATE(fecha_cita) = CURRENT_DATE + INTERVAL '1 day'
            ORDER BY fecha_cita
        """)
        ...
        cur.execute("""
            SELECT nombre AS nombre_cliente, telefono, fecha_cita, estatus, notas
            FROM citas WHERE DATE(fecha_cita) > CURRENT_DATE AND DATE(fecha_cita) <= CURRENT_DATE + INTERVAL '7 days'
            ORDER BY fecha_cita
        """)
        ...
        if momento == "manana":
            ...
            if citas_hoy:
                for c in citas_hoy:
                    ...
                    hora = str(c.get("fecha_cita", ""))[11:16]
                    caliente = " 🔥" if "CALIENTE" in str(c.get("notas") or "") else ""
        ...
        encolar_notificacion(ADMIN_PHONE, msg, tipo=f"resumen_{momento}")
